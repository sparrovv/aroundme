...
export const defaultConfig: ScoreConfig[] = [
  ...
  {
    placeType: "primary school",
    weight: 3,
    distanceType: "walking",
    thresholdInMinutes: 15,
  },
  {
    placeType: "przedszkole",
    weight: 3,
    distanceType: "walking",
    thresholdInMinutes: 15,
  },
  {
    placeType: "ośrodek zdrowia",
    weight: 4,
    distanceType: "walking",
    thresholdInMinutes: 15,
  },
  {
    placeType: "szkoła podstawowa",
    weight: 4,
    distanceType: "walking",
    thresholdInMinutes: 15,
  },
...
export const calculateScore = (
  groupedLocations: GroupedPOIs,
  config: ScoreConfig[] = defaultConfig,
) => {
    ...
    const calculateScoreForPlace = (
      ...
      const numberOfPlacesWithinThreshold = places.filter((p) => {
        if (config.distanceType === "walking") {
          return (
            p.walkingDurationInMinutes &&
            p.walkingDurationInMinutes < config.thresholdInMinutes
          );
        }
      ...
      if (numberOfPlacesWithinThreshold === 1) {
        ...
        return {
          score: config.weight + config.weight / 2,
          count: numberOfPlacesWithinThreshold,
        };
      }
    };
